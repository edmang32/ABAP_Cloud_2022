CLASS zcl_books_data_efmg DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_books_data_efmg IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

    DATA:
      lt_acccateg  TYPE TABLE OF ztb_acccategefmg,
      lt_catego    TYPE TABLE OF ztb_catego_efmg,
      lt_clientes  TYPE TABLE OF ztb_clientesefmg,
      lt_clnts_lib TYPE TABLE OF ztb_clntslibefmg,
      lt_libros    TYPE TABLE OF ztb_libros_efmg.

*   Categorías / Tipo Acceso ztb_acccategefmg
    lt_acccateg = VALUE #(
    ...
    DELETE FROM ztb_acccategefmg.
    INSERT ztb_acccategefmg FROM TABLE @lt_acccateg.
    out->write(
      EXPORTING
        data   = sy-dbcnt
        name   = 'Categorías/Tipo Acceso'
      RECEIVING
...
*   Categorías - Descripción
    ...
    ( bi_categ ='D' descripcion ='Ciencias naturales (Biología, Astronomía, Física)')
    ( bi_categ ='E' descripcion ='Ciencias aplicadas (Tecnología, Informática)')
    ( bi_categ ='F' descripcion ='Arte, Música, Deportes')
    ( bi_categ ='G' descripcion ='Lengua, Poesía, Teatro')
    ( bi_categ ='H' descripcion ='Geografía, Historia') ).
    DELETE FROM ztb_catego_efmg.
    INSERT ztb_catego_efmg FROM TABLE @lt_catego.
    out->write(
      EXPORTING
        data   = sy-dbcnt
        name   = 'Categorías/Descripción'
      RECEIVING
        output = lv_output ).
    ...
    DELETE FROM ztb_clientesefmg.
    INSERT ztb_clientesefmg FROM TABLE @lt_clientes.
    out->write(
      EXPORTING
        data   = sy-dbcnt
        name   = 'Clientes'
    ...
    DELETE FROM ztb_clntslibefmg.
*   INSERT ztb_clntslibefmg FROM TABLE @lt_clnts_lib.
    out->write(
          EXPORTING
            data   = sy-dbcnt
            name   = 'Clientes/Libros'
          RECEIVING
            output = lv_output ).
    ...
    'https://eltinteroeditorial.com/wp-content/uploads/2017/06/c%C3%B3mo-es-el-proceso-de-edici%C3%B3n.jpg' ) ).
    DELETE FROM ztb_libros_efmg.
    INSERT ztb_libros_efmg FROM TABLE @lt_libros.
    out->write(
             EXPORTING
               data   = sy-dbcnt
               name   = 'Libros'
